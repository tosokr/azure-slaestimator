...
export class SLAestimation extends Component {
    ...
    static renderSlaEstimationTier(tier, services, tiers, categories, deleteEstimationTier, expandCollapseEstimationTier,
        deleteEstimationCategory, deleteEstimationService, expandCollapseEstimationCategory, calculateTierSla, calculateDownTime, calculateDownTimeYear,selectRegion) {
        return (
            <div key={tier}>
                <SLAestimationTier tierName={tier} tierServices={services}
                    ...
                    calculateDownTimeYear={calculateDownTimeYear}
                />
            </div>
        );
    }
            ...
            for (var i = 0; i < tiers.length; i++) {
                ...
                if (tierServices.length > 0) {
                    var tierContent = SLAestimation.renderSlaEstimationTier(tiers[i], tierServices,
                        ...
                        this.props.calculateDownTimeYear,
                ...
                <div>
                    ...
                    <div>
                        ...
                        <div className="estimation-totals-panel">
                            ...
                            <div className="estimation-total-label"><p>Maximum acceptable downtime /Month: {Math.floor(this.props.downTime)}m {Math.round((this.props.downTime - Math.floor(this.props.downTime))*60)}s </p></div>
                            <div className="estimation-total-label"><p>Maximum acceptable downtime /Year: {Math.floor(this.props.downTimeYear)}m {Math.round((this.props.downTimeYear - Math.floor(this.props.downTimeYear)) * 60)}s </p></div>
                            <br />
                            ...
                            <div className="estimation-total-label"><p>Maximum acceptable downtime /Month: {Math.floor(this.props.downTimeMultiRegion)}m {Math.round((this.props.downTimeMultiRegion - Math.floor(this.props.downTimeMultiRegion)) * 60)}s </p></div>
                            <div className="estimation-total-label"><p>Maximum acceptable downtime /Year: {Math.floor(this.props.downTimeMultiRegionYear)}m {Math.round((this.props.downTimeMultiRegionYear - Math.floor(this.props.downTimeMultiRegionYear)) * 60)}s </p></div>
                            <br />
