...
export class SLAestimationTier extends Component {
    ...
    render() {
        if (this.props.tierServices.length > 0) {
            ...
            const downTimeYear = this.props.calculateDownTimeYear(tierSla);
                ...
                <div>
                    ...
                    <div className="div-show">
                        ...
                        <div className="estimation-totals-panel">
                            ...
                            <div className="tier-estimation-total-label"><p>Maximum acceptable downtime /Month: {Math.floor(downTime)}m {Math.round((downTime - Math.floor(downTime)) * 60)}s </p></div>
                            <div className="tier-estimation-total-label"><p>Maximum acceptable downtime /Year: {Math.floor(downTimeYear)}m {Math.round((downTimeYear - Math.floor(downTimeYear)) * 60)}s </p></div>
                        </div>
                    </div>
                </div>
            );
